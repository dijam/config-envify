...
const { envify, typeify, upperUnderscore } = require('../config-envify');

describe('Envify tests', () => {
  describe('#upperUnderscore', () => {
    it('should return text uppercased with _ instead of dot', () => {
      expect(upperUnderscore('tests.are.going.well')).be.equal('TESTS_ARE_GOING_WELL');
    });

    it('should return text uppercased with _ instead of dot when the text contains _', () => {
      expect(upperUnderscore('envify_tests.are.going.well')).be.equal('ENVIFY_TESTS_ARE_GOING_WELL');
    });

    it('should return text uppercased with _ instead of dot when the text contains camelCase', () => {
      expect(upperUnderscore('envifyTests.are.going.well')).be.equal('ENVIFYTESTS_ARE_GOING_WELL');
    });
  });
